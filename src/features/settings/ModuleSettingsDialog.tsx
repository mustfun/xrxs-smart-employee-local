...
import { ProjectDialog } from '../chat/ProjectDialog'
...
export function ModuleSettingsDialog({ isOpen, onClose }: ModuleSettingsDialogProps) {
  ...
  // ProjectDialog state
  const [browseModuleId, setBrowseModuleId] = useState<string | null>(null)
  ...
  const handleBrowse = useCallback((moduleId: string) => {
    setBrowseInitialPath(draft[moduleId] ?? '')
    setBrowseModuleId(moduleId)
  }, [draft])

  const handleBrowseSelect = useCallback((path: string) => {
    if (browseModuleId) {
      setDraft(prev => ({ ...prev, [browseModuleId]: path }))
    }
    setBrowseModuleId(null)
  }, [browseModuleId])
  ...
  return (
    <>
      ...
      {/* Reuse ProjectDialog for directory browsing */}
      <ProjectDialog
        isOpen={browseModuleId !== null}
...
interface ModuleRowProps {
  ...
  onBrowse: (moduleId: string) => void
}

function ModuleRow({ module, currentPath, savedDirectories, onPathChange, onSelectSaved, onBrowse }: ModuleRowProps) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative">
        ...
        {/* Dropdown for saved directories + browse option */}
        {showDropdown && (
          <div className="absolute top-full left-0 right-0 mt-1 z-50 glass-alt border border-border-200/60 rounded-lg shadow-lg overflow-hidden">
            ...
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2 text-left text-[length:var(--fs-sm)] text-accent-main-100 hover:text-accent-main-200 hover:bg-bg-200/50 transition-colors border-t border-border-200/30"
            >
